...
import {validateEntry,convertEntry,calculateTotalDeduction} from './DeductionLogic'
...
const DeductionBy80C = (props) => {
    ...
    const addDeduction = () => {
        ...
        if (validateEntry(deductionDetails)) {
            const totalDeduction = calculateTotalDeduction(convertEntry(deductionDetails),MAX_80_C_LIMIT)
            if (totalDeduction === 0) {
